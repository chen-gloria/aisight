        ...
        try:
            ...
            if "predictions" in detection_data:
                ...
                formatted_results.append(
                    {"detections": detections, "image_shape": image_shape}
                )
            ...
            return {
                ...
                "total_detections": sum(
                    len(r["detections"]) for r in formatted_results
                ),
                "source": image_data if is_path else "base64_image",
    ...
    try:
        ...
        try:
            ...
            if "predictions" in segmentation_data:
                ...
                formatted_results.append(
                    {"segments": segments, "image_shape": image_shape}
                )
    ...
    try:
        ...
        try:
            ...
            if "predictions" in classification_data:
                ...
                formatted_results.append(
                    {"classifications": classifications, "image_shape": image_shape}
                )
    ...
    try:
        ...
        try:
            ...
            if "predictions" in tracking_data:
                ...
                formatted_results.append({"tracks": tracks, "image_shape": image_shape})
        ...
        return {"error": f"Failed to track objects: {str(e)}"}
...
def train_model(
    ...
    logger.info(
        f"Starting model training with {epochs} epochs - this may take a while..."
    )
    ...
    return {
        ...
        "training_log_sample": result.get("stdout", "")[:1000] + "..."
...
def validate_model(
    ...
    return {
        "status": "success",
        "metrics": metrics,
        "validation_output": result.get("stdout", "")[:1000] + "..."
        if len(result.get("stdout", "")) > 1000
        else result.get("stdout", ""),
    }
...
def export_model(
    ...
    return {
        ...
        "format": format,
        "export_output": result.get("stdout", "")[:1000] + "..."
        if len(result.get("stdout", "")) > 1000
        else result.get("stdout", ""),
    }
...
def camera_detection_thread(model_name, confidence, fps_limit=30, camera_id=0):
    ...
    try:
        ...
        output_dir = os.path.join(
            tempfile.gettempdir(), f"yolo_camera_{int(time.time())}"
        )
        ...
        # Log camera start
        logger.info(
            f"Starting camera detection with model {model_name}, camera ID {camera_id}"
        )
        detection_results.append(
            {
                "timestamp": time.time(),
        ...
        try:
            check_result = subprocess.run(
                check_cmd, capture_output=True, text=True, check=False, timeout=10
            )
        ...
        try:
            ...
            if process.poll() is not None:
                error_msg = (
                    f"YOLO process failed to start (exit code {process.returncode})"
                )
            ...
            camera_startup_status = {"success": True, "timestamp": time.time()}
                    ...
                    if json_start >= 0:
                        ...
                        if "predictions" in detection_data:
                            ...
                            if frame_count % 30 == 0:
                                ...
                                logger.info(
                                    f"Camera running: processed {frame_count} frames ({fps:.1f} FPS)"
                                )
                                detection_count = sum(
                                ...
                                logger.info(
                                    f"Total detections in current buffer: {detection_count}"
                                )
...
def get_camera_detections() -> Dict[str, Any]:
    ...
    if (
        not camera_running
        and camera_startup_status
        and not camera_startup_status.get("success", False)
    ):
        return {
            "status": "error",
            "message": "Camera detection failed to start",
...
def test_connection() -> Dict[str, Any]:
    ...
    try:
        ...
        yolo_version = (
            version_result.get("stdout", "Unknown")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if test_result["success"]:
            ...
            logger.warning(
                "Service may not function correctly without YOLO CLI available"
            )
    except Exception as e:
