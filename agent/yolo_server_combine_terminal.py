...
def track_objects(
    ...
    try:
        ...
        return {"error": f"Failed to generate tracking command: {str(e)}"}
...
def start_camera_detection(
    ...
    return {
        ...
        "parameters": {"confidence": confidence, "camera_id": camera_id, "task": task},
    }
...
def comprehensive_image_analysis(
    ...
    if "command" in detect_result:
        commands.append(
            {"task": "object_detection", "command": detect_result["command"]}
        )
    ...
    try:
        ...
        if "command" in cls_result:
            commands.append(
                {"task": "classification", "command": cls_result["command"]}
            )
    except Exception as e:
    ...
    for directory in CONFIG["model_dirs"]:
        ...
        if os.path.exists(pose_model_path):
            ...
            commands.append({"task": "pose_detection", "command": result["command"]})
            break
    ...
    return {
        ...
        "parameters": {"confidence": confidence, "save_results": save_results},
    }
